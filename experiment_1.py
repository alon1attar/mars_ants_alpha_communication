...
class Navigator(RobotBase):
    ...
    def run_all_navigator(self, lock):
        ...
        print("Graph Mine Found Over Time: ", graph_mine_found)
        print("Graph Mine Done Over Time: ", graph_mine_done)
...
class Shoveler(RobotBase):
    ...
    def state_move_to_mine(self, x, y, lock):
        # Moves only one step at a time since the function will probably be called in a while loop
        if self.x - x > 0:
            if grid_move_checker(self.grid, self.get_pos_on_grid()[0], self.get_pos_on_grid()[1],
