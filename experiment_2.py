...
class Navigator(RobotBase):
    ...
    def find_next_mine_2(self, lock):
        ...
        while (True):
            ...
            if mines_in_view > 0:
                for i in range(mines_in_view):
                    x = self.x - (vals.view // 2) + mine_loc_rel[0][i]
                    y = self.y - (vals.view // 2) + mine_loc_rel[1][i]
    ...
    def communicate_D_S(self):
        ...
        if len(rangeRobots) > 1:
            for robot in rangeRobots:
                ...
                elif robot.robot_type == 2:
                    if robot == digger_2:
                        for place in self.mine_found:
                            if place not in robot.taskFound and place not in digger_1.taskFound and place not in digger_3.taskFound:
                                robot.taskLocationsQueue.append(place)
                                robot.taskFound.append(place)
                    elif robot == digger_1:
                        for place in self.mine_found:
                            if place not in robot.taskFound and place not in digger_2.taskFound and place not in digger_3.taskFound:
                                robot.taskLocationsQueue.append(place)
                                robot.taskFound.append(place)
                    elif robot == digger_3:
                        for place in self.mine_found:
                            if place not in robot.taskFound and place not in digger_2.taskFound and place not in digger_1.taskFound:
                                robot.taskLocationsQueue.append(place)
                                robot.taskFound.append(place)
...
# All of the Shoveler class was written by me
...
# All of the Digger class was written by me
class Digger(RobotBase):
    ...
    def state_idle(self):
        # Currently when the taskListLocations is empty
        if len(self.taskLocationsQueue) > 0:
            self.phase = 1
        else:
            pass
...
if __name__ == "__main__":
    ...
    digger_2 = Digger(environment1, 3)
    digger_3 = Digger(environment1, 4)
    ...
    t4 = threading.Thread(target=digger_2.run_all_digger, args=[lock])
    t5 = threading.Thread(target=shoveler_1.run_all_shoveler, args=[lock])
    t6 = threading.Thread(target=digger_3.run_all_digger, args=[lock])
